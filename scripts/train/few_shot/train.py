...
def main(opt):
    ...
    data = data_utils.load(opt, ['train', 'val'])  # 返回两个dataloader
    ...
    meters = {'train': {field: AverageValueMeter() for field in opt['log.fields']}}  # 直接拷贝源码
    ...
    def on_end_epoch(hook_state, state):
        ...
        if val_loader is not None:
            model_utils.evaluate(opt, state['model'],
                                 val_loader,
                                 meters['val'],
        ...
        if val_loader is not None:
            if meter_vals['val']['loss'] < hook_state['best_loss']:
                ...
                paddle.save(state['model'].state_dict(), os.path.join(opt['log.exp_dir'], 'best_model.pdparams'))
            ...
            paddle.save(state['model'], os.path.join(opt['log.exp_dir'], 'best_model.pdparams'))

        state['scheduler'].step(meter_vals['val']['loss'])
    ...
    engine.train(
        ...
        optim_config={'learning_rate': opt['train.learning_rate'],
